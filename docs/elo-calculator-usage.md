...
3. **Human/AI eligibility filter** — if user-submitted games ever become a data source (not currently), only human-vs-human results should be rateable. Maps onto the app's existing `scope: vs_ai`/`mp` classification (`cloud/src/games-scope.ts`) — no new schema needed, just wiring it in as a filter, not a weight.
4. **2-player vs. 3+ player architecture fork** — same trigger (user-submitted multiplayer as a data source). `calculate_elo_delta()` is strictly pairwise, which is correct for tournament matches (always exactly 2 players by construction) but doesn't apply to a 3+ player FFA without a different algorithm (e.g., pairwise decomposition, or a placement-based system) or explicit exclusion.
...
7. **Reference these docs from root `CLAUDE.md`** — `docs/elo-calculator-usage.md`/`docs/elo-calculator-design.md` aren't listed in its "Key docs & skills" section, unlike `docs/tournament-rules.md` and similar. Worth a conscious call once this branch merges — these are scoped as operator scripts rather than core app docs, so it may be intentional, but that hasn't actually been decided either way.
